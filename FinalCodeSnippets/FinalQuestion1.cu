...
__global__ void smooth_shared(float * v_new, const float * v) {
    ...
    int numThreads = blockDim.x * gridDim.x;
    if(myIdx == 0) {
        s[blockDim.x+1] = v[myIdx];
        s[tIdx] = v[myIdx];
    }
    else if(myIdx == numThreads-1) {
        s[blockDim.x] = v[myIdx];
        s[tIdx] = v[myIdx];
    }
    else if(tIdx == 0) {
        s[blockDim.x+1] = v[myIdx-1];
        s[tIdx] = v[myIdx];
    }
    else if(tIdx == blockDim.x-1) {
        s[blockDim.x] = v[myIdx+1];
        s[tIdx] = v[myIdx];
    }
    else {
        s[tIdx] = v[myIdx];
    ...
    __syncthreads();
    float myElt = s[tIdx];
    float myLeftElt=(tIdx==0)?s[blockDim.x+1]:s[tIdx-1];
    float myRightElt=s[tIdx+1];
    v_new[myIdx] = 0.25f * myLeftElt + 0.5f * myElt + 0.25f * myRightElt;
}
